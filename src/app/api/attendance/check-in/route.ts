...
export async function POST(request: Request) {
  ...
  // Policy: reject attendance outside the office radius or with low GPS accuracy.
  ...
  let status: string = resolveBaseStatus(ev);

  // LATE only overrides when the employee is otherwise PRESENT.
  const scheduleShift = await findActiveScheduleShift(
    db,
    user.organizationId,
    emp.id,
    serverTime
  );
  if (status === "PRESENT" && scheduleShift && isLate(serverTime, scheduleShift)) {
    status = "LATE";
  }
  ...
  const [record] = await db
    ...
    .values({
      ...
      scheduleId: scheduleShift?.scheduleId ?? null,
      type: "CHECK_IN",
